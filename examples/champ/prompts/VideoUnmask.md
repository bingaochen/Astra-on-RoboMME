...
TASK RULES
The demonstration already showed which colored cube became hidden by each container. Recover that association from the video. At execution, pick the container hiding the first color requested by the instruction. Do not press a button or replay the covering phase. If a second color is requested, release the first container, then pick the second. Use the current scene to locate each container; the required object is the covering container, not the hidden cube. No swap is part of this task.
...
Instruction: reveal blue then green. History: picked container hiding blue. Output: put down the container
