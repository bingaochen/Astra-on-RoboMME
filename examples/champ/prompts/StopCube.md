...
TASK RULES
Prepare above the button first without pressing. Then remain static while the moving cube makes the required visits, and press to stop it on the requested visit. Ground the preparation point on the button, but the final template's point on the TARGET, matching the dataset grammar. A completed 'remain static' is a completed waiting segment, NOT proof of one target crossing. Do not equate their count with visits or infer an absolute simulator schedule from frame IDs. With only a current still and waiting history, the required crossing may be unidentifiable; do not claim accurate timing from this input. Use remain static while evidence does not justify pressing. No extra event counter or hidden state is supplied.
...
History: preparation completed. Current still shows the cube away from the target; the requested visit is not established. Output: remain static
