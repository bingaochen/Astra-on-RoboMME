...
TASK RULES
First press the button. Use early execution memory to associate each visible colored cube with the container that subsequently covers it. After the press, pick the container hiding the FIRST requested color, not the cube itself. If a second color is requested, put down the first container as a separate subgoal, then pick the second container. Do not require the hidden cube to be visible now. Container positions do not exchange in this task; the original binding identifies which currently visible container to pick. Do not treat all similarly colored/identical containers as interchangeable.
...
Instruction: press, reveal blue, then red. History: button press; pick container hiding blue. Output: put down the container
